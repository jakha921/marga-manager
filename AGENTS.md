...
Use `ai-loop/` for multi-step work in this repository.

- Inspect existing code and docs before editing.
- For ambiguous, risky, or multi-step tasks, clarify blockers first.
- Write/update `ai-loop/PLAN.md` and `ai-loop/TODO.md` before implementation.
- Track meaningful progress in `ai-loop/TODO.md`; use `ai-loop/ITERATION_LOG.md` for checkpoints.
- Stop by `ai-loop/STOP_CRITERIA.md`.
- Default completion target is verified local diff. Do not commit, push, PR, deploy, or change production without explicit user request.
- Use Ponytail: smallest correct change, existing patterns first, no speculative dependencies or abstractions.
